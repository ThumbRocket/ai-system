...
class PEControl(Elaboratable):
    def __init__(
        self, num_bits, width, g_depth, l_depth, sys_size_bit, signed=True
    ):
    ...
    def elaborate(self, platform):
        ...
        for i in range(self.sys_size):
            ...
            m.d.comb += [
                ...
                self.lb_b[i].w_data.eq(
                    Mux(self.reuse_b, self.lb_b[i].r_data, self.in_r_data)
                ),
            ...
            for j in range(self.sys_size):
                ...
                m.d.comb += [
                    ...
                    self.pe[i][j].in_act.eq(self.act_code),
                    self.pe[i][j].in_init.eq(self.pe_init),
                    self.row_ortree[i]
                    .in_arr[j]
                    .eq(
                        Mux(
                            self.sys_sel_row[i] & self.sys_sel_col[j],
                            self.pe[i][j].out_d,
                            0,
                        )
                    ),
        ...
        with m.FSM(reset="INIT"):
            with m.State("INIT"):
                ...
                with m.Else():
                    m.d.sync += [
                        self.addr_io.eq(0),
                        self.addr_io_ovf.eq(0),
                        self.next_pc.eq(1),
                        self.next_pc_ovf.eq(0),
                    ]
            with m.State("FETCH"):
                m.d.sync += [
                    self.out_w_en.eq(0),
                    self.fan_cnt.eq(0),
                    self.fan_cnt_ovf.eq(0),
                    self.lb_a_r_en.eq(0),
                    self.lb_b_r_en.eq(0),
                    self.lb_a_w_en.eq(0),
                    self.lb_b_w_en.eq(0),
                    self.sys_sel_row.eq(0),
                    self.sys_sel_col.eq(0),
                    self.reuse_b.eq(0),
                ]
                with m.If(self.addr_io == self.next_pc):
                    m.next = "DECODE"
                    m.d.sync += [
                        Cat(self.next_pc, self.next_pc_ovf).eq(
                            self.next_pc + 1
                        ),
                    ]
                with m.Else():
            ...
            with m.State("DECODE"):
                with m.Switch(self.opcode):
                    with m.Case(OPCODE.LOAD):
                        ...
                        m.next = "LOAD"
                        m.d.sync += [
                            self.addr_io.eq(self.v2),
                            self.addr_io_ovf.eq(0),
                            self.sel_a.eq(~self.v1[0]),
                            self.sel_b.eq(self.v1[0]),
                            self.sys_sel_row.eq(~self.v1[0]),
                            self.sys_sel_col.eq(self.v1[0]),
                            self.fan_cnt.eq(-1),
                        ]
                    ...
                    with m.Case(OPCODE.STORE):
                        ...
                        m.d.sync += [
                            ...
                            self.addr_io.eq(self.v2),
                            self.addr_io_ovf.eq(0),
                            Cat(self.magic_cnt, self.magic_cnt_ovf).eq(
                                self.magic_cnt + 1
                            ),
                            self.out_w_en.eq(1),
                    ...
                    with m.Case(OPCODE.SET_M):
                        ...
                        m.d.sync += [
                            self.fan_in.eq(
                                Mux(self.v2_f.any(), self.v2_f, self.fan_in)
                            ),
            ...
            with m.State("LOAD"):
                ...
                with m.If(self.fan_cnt_next == self.fan_in):
                    ...
                    with m.If(
                        self.sys_sel_row & self.sys_h_sel
                        | self.sys_sel_col & self.sys_w_sel
                    ):
                        m.next = "FETCH"
                        m.d.sync += [
                            self.lb_a_w_en.eq(0),
...
if __name__ == "__main__":
    ...
    def store(sys_h, sys_w, start_addr):
        code = make_code(OPCODE.STORE, 0, start_addr)
    ...
    def bench():
        ...
        for i in range(num_runs):
            ...
            yield from load(
                LOAD_DEST.A, lb_a, fan_in * (sys_h + 1), start_addr=8
            )
            if is_first:
                yield from load(
                    LOAD_DEST.W, lb_b, fan_in * (sys_w + 1), start_addr=12
                )
