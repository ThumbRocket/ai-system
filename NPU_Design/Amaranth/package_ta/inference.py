        ...
        if w is not None:
            if fan_in != self.last_fan_in:
                set_m = make_code(
                    OPCODE.SET_M,
                    0,
                    # switching (sys_h, sys_w) order because of transpose
                    # hardware : x @ w.T
                    # here     : (w @ x.T).T
                    (self.sys_w << (self.cnt_bits + self.sys_size_bit))
                    | (self.sys_h << self.cnt_bits)
                    | fan_in,
                )
            ...
            self._append_code(make_code(OPCODE.LOAD, LOAD_DEST.W, 0))  # addr
            self.data_w.append(w)

        self._append_code(make_code(OPCODE.LOAD, LOAD_DEST.A, 0))  # addr
        self._append_code(make_code(OPCODE.EXEC, FIFO.REUSE << 2, 0))
        self._append_code(make_code(OPCODE.STORE, 0, 0))  # addr
