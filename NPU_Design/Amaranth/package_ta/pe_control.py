...
class PEControl(Elaboratable):
    def __init__(self, num_bits, width, g_depth, l_depth, signed=True):
    ...
    def elaborate(self, platform):
        ...
        m.d.comb += [
            ...
            pipe_b.w_data.eq(Mux(self.reuse_b, pipe_b.r_data, self.in_r_data)),
        ...
        with m.FSM(reset="INIT"):
            ...
            with m.State("LOAD"):
                m.d.sync += [
                    ...
                    pipe_a.w_en.eq(self.sel_a),
                    pipe_b.w_en.eq(self.sel_b),
                ]
...
if __name__ == "__main__":
    ...
    dut = PEControl(
        unit_width, width, g_depth=g_depth, l_depth=l_depth, signed=signed
    )
    ...
    def bench():
        ...
        for i in range(num_runs):
            ...
            data_a = np.random.randint(low=0, high=2**width, size=[fan_in])
            for d in data_a:
                lb_a.append(d)
